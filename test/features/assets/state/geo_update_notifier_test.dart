...
void main() {
  ...
  group('GeoUpdateNotifier.addRecommended', () {
    test('调用后 geoip / geosite 应同时立刻进入 isUpdating:true —— '
        '证明真的触发了 update()，而不是把 state 用 ?? 现有值重建一遍的恒等占位操作', () async {
    ...
    test('CDN URL 未配置（当前构建下 GeoAsset.geoip/geosite.cdnUrl 恒为空串）时，'
        '两个资源最终应收敛为 error 状态，而不是卡在 isUpdating 或抛出未捕获异常', () async {
      await _mockPathProvider();
      final notifier = GeoUpdateNotifier(await _testRef());
      await Future<void>.delayed(const Duration(milliseconds: 50));
